...
def main():
    ...
    result = runtime.life_loop.tick(
        timedelta(hours=3, minutes=30)
    )

    status = build_life_status(
        runtime.life_state
    )

    print("=== 小七 Agent Runtime ===")
    print(
        f"模拟时间：{start} -> {runtime.current_time}"
    )
    print(
        f"经过的 Slot：{result.slots_seen}"
    )
